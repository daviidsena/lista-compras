...
if (!empty($_POST)) {
    ...
    if ($validacao) {
        ...
        try {
            $pdo = Banco::conectar();
            $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $sql = "UPDATE produtos SET  nome = ?, quantidade = ?, id_unidade = ?, preco = ?, id_categoria = ? WHERE id = ?";
            $q = $pdo->prepare($sql);
            $q->execute(array($nome, $quantidade, $unidade, $preco, $categoria, $id));

            Banco::desconectar();
        } catch (PDOException $e) {
            echo 'Error ao editar o item: ' . $e->getMessage();
        }
